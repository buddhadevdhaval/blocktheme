...
$ambrygen_terms = $ambrygen_has_query ? Helper::get_terms_by_name_or_meta_like($ambrygen_taxonomy, $ambrygen_tokens, 'isoform', 50) : [];
...
<div <?php echo wp_kses_data($ambrygen_wrapper_attributes); ?>>
	...
	<div class="genes-table__search">
		<form method="get" class="genes-table__search-form">
			...
			<input id="<?php echo esc_attr($ambrygen_block_id ?: 'genes-table'); ?>-search" type="search"
				class="genes-table__search-input" name="symbols" value="<?php echo esc_attr($ambrygen_symbols); ?>"
				placeholder="<?php echo esc_attr((string) $ambrygen_placeholder); ?>" autocomplete="off" />
			<input type="submit" class="genes-table__search-button"
				value="<?php esc_attr_e('Search', 'ambrygen-web'); ?>" />
		</form>
	</div>

	<?php if ('' === $ambrygen_symbols && '' !== trim((string) $ambrygen_instruction_text)): ?>
		<div class="is-style-gl-s24" aria-hidden="true"></div>
		<div class="genes-table__instruction"><?php echo esc_html((string) $ambrygen_instruction_text); ?></div>
	<?php endif; ?>


	<div class="gl-data-table gl-data-table--cols-5">
			...
			<div class="gl-data-table__grid">
				...
				<?php if (empty($ambrygen_terms)): ?>
					<div class="gl-data-table__row">
						<div class="gl-data-table__cell gl-data-table__cell--name data-not-found" data-label="Gene">
							<?php echo esc_html((string) $ambrygen_no_results_text); ?>
						</div>
					</div>
				<?php else: ?>
					<?php foreach ($ambrygen_terms as $ambrygen_term): ?>
						...
						?>
						<div class="gl-data-table__row">
							<div class="gl-data-table__cell gl-data-table__cell--name" data-label="Gene">
							...
							<div class="gl-data-table__cell" data-label="Covered CDS Count">
								<?php echo esc_html($ambrygen_cds_covered); ?>
							</div>
							<div class="gl-data-table__cell" data-label="Total CDS"><?php echo esc_html($ambrygen_cds_count); ?>
							</div>
							<div class="gl-data-table__cell" data-label="% of CDS Covered">
								<?php echo esc_html($ambrygen_percentage); ?>
							</div>
						</div>
					<?php endforeach; ?>
				<?php endif; ?>
