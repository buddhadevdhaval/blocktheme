...
if ( ! defined( 'ABSPATH' ) ) {
	...
	$ambrygen_sort_grid_tests = static function ( array $ambrygen_posts, array $ambrygen_ordered_ids = array() ): array {
		if ( empty( $ambrygen_posts ) || empty( $ambrygen_ordered_ids ) ) {
			return $ambrygen_posts;
		}

		$ambrygen_ordered   = array();
		$ambrygen_remaining = array();

		foreach ( $ambrygen_posts as $ambrygen_post ) {
			$ambrygen_remaining[ (int) $ambrygen_post->ID ] = $ambrygen_post;
		}

		foreach ( $ambrygen_ordered_ids as $ambrygen_target_id ) {
			$ambrygen_target_id = absint( $ambrygen_target_id );
			if ( isset( $ambrygen_remaining[ $ambrygen_target_id ] ) ) {
				$ambrygen_ordered[] = $ambrygen_remaining[ $ambrygen_target_id ];
				unset( $ambrygen_remaining[ $ambrygen_target_id ] );
			}
		}

		return array_merge( $ambrygen_ordered, array_values( $ambrygen_remaining ) );
	};
				...
				<div class="tabs__panels">
					<?php foreach ( $ambrygen_selected_tabs as $ambrygen_index => $ambrygen_tab_data ) : ?>
						<?php
							...
							$ambrygen_cache_key = 'gtg_tab_posts_' . md5( (string) wp_json_encode( $ambrygen_query_args ) );
							...
							if ( false === $ambrygen_cached ) {
								...
								$ambrygen_tab_posts   = is_array( $ambrygen_tests_query->posts ) ? $ambrygen_tests_query->posts : array();
								wp_cache_set( $ambrygen_cache_key, $ambrygen_tab_posts, 'ambrygen_blocks', HOUR_IN_SECONDS );
							} else {
								$ambrygen_tab_posts = is_array( $ambrygen_cached ) ? $ambrygen_cached : ( isset( $ambrygen_cached->posts ) ? $ambrygen_cached->posts : array() );
							}

							$ambrygen_ordered_ids = isset( $ambrygen_tab_data['orderedPostIds'] ) && is_array( $ambrygen_tab_data['orderedPostIds'] )
								? array_map( 'absint', $ambrygen_tab_data['orderedPostIds'] )
								: array();

							$ambrygen_sorted_posts = $ambrygen_sort_grid_tests( $ambrygen_tab_posts, $ambrygen_ordered_ids );
							$ambrygen_total_tests  = count( $ambrygen_sorted_posts );
							?>
						<div class="tabs__panel<?php echo esc_attr( $ambrygen_is_active ); ?>"
							...
							<div class="features-tabs__grid">
								<?php if ( ! empty( $ambrygen_sorted_posts ) ) : ?>
									<?php $ambrygen_post_index = 0; ?>
									<?php foreach ( $ambrygen_sorted_posts as $ambrygen_post ) : ?>
										<?php
											$ambrygen_post_id       = $ambrygen_post->ID;
											$ambrygen_post_title    = get_the_title( $ambrygen_post_id );
											$ambrygen_post_terms    = get_the_terms( $ambrygen_post_id, $ambrygen_taxonomy );
										...
										<div class="features-tabs__card<?php echo esc_attr( $ambrygen_is_extra_test ? ' features-tabs__card--is-extra is-view-all-hidden' : ' js-gsap-fade' ); ?>"
											...
											<?php if ( $ambrygen_is_extra_test ) : ?>style="display:none;"<?php endif; ?>
											...
											<a class="features-tabs__view-link site-btn is-style-site-text-btn has-right-arrow"
												href="<?php echo esc_url( get_permalink( $ambrygen_post_id ) ); ?>"
