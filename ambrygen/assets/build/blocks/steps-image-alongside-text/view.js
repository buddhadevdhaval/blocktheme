/******/ (() => { // webpackBootstrap
/*!**************************************************************!*\
  !*** ./assets/src/blocks/steps-image-alongside-text/view.js ***!
  \**************************************************************/
// Vertical Tabs
document.addEventListener('DOMContentLoaded', () => {
  document.querySelectorAll('.vertical-tabs').forEach(vTabsContainer => {
    const vItems = Array.from(vTabsContainer.querySelectorAll('.vertical-tabs__item'));
    if (vItems.length === 0) {
      return;
    }
    ...
    const setActiveTab = (nextIndex, shouldFocus = false) => {
      vItems.forEach((item, index) => {
        ...
        const panel = item.querySelector('.vertical-tabs__content[role="tabpanel"]');
    ...
    const getItemIndexFromEvent = eventTarget => vItems.findIndex(item => item.contains(eventTarget));
    setActiveTab(0);
    ...
    vTabsContainer.addEventListener('keydown', event => {
      const currentTab = event.target.closest('.vertical-tabs__header[role="tab"]');
      ...
      switch (event.key) {
        case 'ArrowUp':
        case 'ArrowLeft':
          nextIndex = (currentIndex - 1 + vItems.length) % vItems.length;
          break;
        case 'ArrowDown':
        case 'ArrowRight':
