/******/ (() => { // webpackBootstrap
/*!********************************************!*\
  !*** ./assets/src/blocks/tab-menu/view.js ***!
  \********************************************/
document.addEventListener('DOMContentLoaded', function () {
  document.querySelectorAll('.secondary-sticky-tabs').forEach(function (wrapper) {
    ...
    const tabBehavior = wrapper.getAttribute('data-tab-behavior') || 'scroll';
    const sectionPairs = [];
    const getTopPosition = function (element) {
      return element.getBoundingClientRect().top + window.pageYOffset;
    };
    const getOffset = function () {
      return parseInt(wrapper.getAttribute('data-offset') || '250', 10);
    };
    const scrollToTarget = function (target) {
      const targetPosition = target.getBoundingClientRect().top + window.pageYOffset - getOffset();
      window.scrollTo({
        top: targetPosition,
        behavior: 'smooth'
      });
    ...
    const resolveTargetElement = function (targetId) {
      ...
      const duplicateMatches = Array.from(document.querySelectorAll(`#${escapedTargetId}`));
      if (duplicateMatches.length <= 1) {
        return firstMatch;
      }
      const wrapperTop = getTopPosition(wrapper);
      const sortedMatches = duplicateMatches.map(function (element, index) {
    ...
    const setActiveTab = function (activeTab) {
      ...
      const isSpacerElement = function (element) {
        ...
        return Array.from(element.classList).some(function (cssClass) {
      ...
      const setManagedVisibility = function (element, shouldShow) {
        if (!element.hasAttribute(originalHiddenAttr)) {
          element.setAttribute(originalHiddenAttr, element.hidden ? 'true' : 'false');
        }
        const wasOriginallyHidden = element.getAttribute(originalHiddenAttr) === 'true';
        if (shouldShow) {
          if (element.getAttribute(managedAttr) === 'true') {
            element.hidden = wasOriginallyHidden;
            element.setAttribute('aria-hidden', wasOriginallyHidden ? 'true' : 'false');
      ...
      sectionPairs.forEach(function (pair) {
        ...
        getAdjacentSpacers(pair.target).forEach(function (spacer) {
          setManagedVisibility(spacer, isActive);
        });
      });
    };
    const defaultPair = sectionPairs.find(function (pair) {
    ...
    const updateActiveTabFromScroll = function () {
      ...
      const wrapperBottom = getTopPosition(wrapper) + wrapper.offsetHeight;
      let currentTab = null;
