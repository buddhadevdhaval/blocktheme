...
(() => {
/*!************************************************!*\
  !*** ./assets/src/blocks/testimonials/view.js ***!
  \************************************************/
__webpack_require__.r(__webpack_exports__);
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  const restoreFocus = element => {
    ...
    const focusTarget = element.matches(focusableSelector) ? element : element.querySelector(focusableSelector);
    if (focusTarget) {
      focusTarget.focus();
      return;
    }
  ...
  const getLiveRegionText = (current, total, rootSection) => {
    const template = rootSection?.dataset.testimonialsStatusTemplate || 'Slide %1$d of %2$d';
  ...
  const updateLiveRegion = (swiper, container, rootSection) => {
    ...
    statusElement.textContent = getLiveRegionText(current, total, rootSection);
  };
  const ensureSwiperControls = (container, rootSection) => {
    const statusId = rootSection?.dataset.testimonialsStatusId;
    const prevLabel = rootSection?.dataset.testimonialsPrevLabel || 'Previous testimonial';
    const nextLabel = rootSection?.dataset.testimonialsNextLabel || 'Next testimonial';
  ...
  const initMobileSwiper = () => {
    const container = document.querySelector('.testimonial_slider');
    const parentGrid = container ? container.closest('.ambry-testimonials__grid') : document.querySelector('.ambry-testimonials__grid');
    if (!parentGrid) {
      return;
    }
    const rootSection = parentGrid.closest('.wp-block-ambrygen-testimonials');
    ...
    if (window.innerWidth > 767) {
      ...
      const wasFocusInSlider = activeElement && container.contains(activeElement);
      const focusedSlide = wasFocusInSlider ? activeElement.closest('.ambry-testimonials__grid__item') : null;
      destroySwiper(container);

      // Get all testimonial items
      const wrapper = container.querySelector('.ambry-testimonials__wrapper, .swiper-wrapper');
      if (wrapper) {
        const slides = Array.from(wrapper.querySelectorAll('.ambry-testimonials__grid__item'));
        ...
        if (wasFocusInSlider) {
          restoreFocus(focusedSlide && focusedSlide.isConnected ? focusedSlide : slides[0]);
        }
    ...
    if (window.innerWidth <= 768) {
      ...
      let swiperContainer = document.querySelector('.testimonial_slider');
      if (!swiperContainer) {
        ...
        swiperWrapper.className = 'ambry-testimonials__wrapper swiper-wrapper';

        // Get all testimonial items from grid
        const items = Array.from(parentGrid.querySelectorAll('.ambry-testimonials__grid__item'));
      ...
      if (!swiperContainer.swiper) {
        const wrapper = swiperContainer.querySelector('.swiper-wrapper, .ambry-testimonials__wrapper');
        ...
        swiperInstance = new swiper__WEBPACK_IMPORTED_MODULE_0__["default"](swiperContainer, {
          modules: [swiper_modules__WEBPACK_IMPORTED_MODULE_1__.Navigation, swiper_modules__WEBPACK_IMPORTED_MODULE_1__.EffectFade, swiper_modules__WEBPACK_IMPORTED_MODULE_1__.Keyboard, swiper_modules__WEBPACK_IMPORTED_MODULE_1__.A11y],
          slidesPerView: 1,
          loop: false,
          effect: 'fade',
          fadeEffect: {
        ...
        swiperInstance.on('slideChange', () => updateLiveRegion(swiperInstance, swiperContainer, rootSection));
