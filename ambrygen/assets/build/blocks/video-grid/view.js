/******/ (() => { // webpackBootstrap
/*!**********************************************!*\
  !*** ./assets/src/blocks/video-grid/view.js ***!
  \**********************************************/
(function () {
  const initVideoGridModal = () => {
    ...
    document.querySelectorAll('.wp-block-ambrygen-gallery.image-grid-block.video-grid').forEach(blockEl => {
      if (blockEl.dataset.videoGridBound === '1') {
        return;
      }
      const videoModal = blockEl.querySelector('[data-video-modal]');
      const videoContainer = blockEl.querySelector('[data-video-modal-container]');
      const videoTitleEl = blockEl.querySelector('[data-video-modal-title]');
      const videoDescriptionEl = blockEl.querySelector('[data-video-modal-description]');
      ...
      if (!videoModal || !videoContainer || !videoTitleEl || !videoDescriptionEl) {
        return;
      }
      ...
      const openVideoModal = element => {
        ...
        if (descEl) {
          videoDescriptionEl.replaceChildren(createSafeDescriptionFragment(descEl));
        } else {
          videoDescriptionEl.replaceChildren();
        }
        if (videoType === 'mp4') {
      ...
      triggers.forEach(trigger => {
        ...
        const titleEl = trigger.closest('.videos__cards-item')?.querySelector('.videos__cards-item-title');
        const triggerLabel = titleEl?.textContent?.trim();
        if (triggerLabel && !trigger.hasAttribute('aria-label')) {
          trigger.setAttribute('aria-label', `Play video: ${triggerLabel}`);
        }
