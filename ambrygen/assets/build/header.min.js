...
document.addEventListener("DOMContentLoaded", function () {
  ...
  let headerInitialSectionHeight = adminBarHeight + (headerSection?.offsetHeight || 0);
  ...
  function adjustHeader() {
    ...
    if (scrollTop >= headerHeight / 2) {
      ...
      headerSection.classList.remove("site-header-sticky", "site-header-show");
  ...
  function setHoverEvent() {
    ...
    menuUlist.addEventListener("mouseover", event => {
      ...
      if (menuItem && menuUlist.contains(menuItem)) {
        const hasMegaDropdown = menuItem.querySelector(".mega-dropdown");
        if (hasMegaDropdown) {
  ...
  function setupToggle(selector, parentClass, onToggle) {
    ...
    buttons.forEach(button => {
      button.addEventListener("click", event => {
        ...
        const siblings = Array.from(item.parentNode.children).filter(el => el !== item && el.classList.contains(parentClass.replace(".", "")));
  ...
  // Initialize toggles
  setupToggle(".menu-item-has-children > .menu-expand", ".menu-item-has-children", (item, isActive) => {
    if (isActive) {
      resetListMenu();
    }
  });
  ...
  if (helloBar) {
    const cookieDays = parseInt(helloBar.getAttribute("data-cookie-days"), 10); // Convert data-cookie-days to integer
