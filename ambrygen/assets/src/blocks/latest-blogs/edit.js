...
function getPostAuthorsData( post ) {
	const authorsData = [];
	...
	if (
		webinarAuthors &&
		Array.isArray( webinarAuthors ) &&
		webinarAuthors.length > 0
	) {
...
function formatAuthorNames( authorsData ) {
	return authorsData
		.map( ( author ) => {
...
export default function Edit( { attributes, setAttributes, clientId } ) {
	const { blockId, title, headingLevel, postsPerPage } = attributes;
	...
	const { posts, categories, tags, isFetching, defaultCatId } = useSelect(
		( select ) => {
			...
			const uncategorizedTerm = getEntityRecords(
				'taxonomy',
				'category',
				{ slug: 'uncategorized' }
			)?.[ 0 ];
			const filteredCategories = allCategories.filter(
				( cat ) => cat.id !== uncategorizedTerm?.id
			);
			...
			return {
				...
				tags: allTags,
				isFetching: isResolving( 'core', 'getEntityRecords', [
					'postType',
					'post',
					queryArgs,
				] ),
	...
	const postsWithFullData = useSelect(
		( select ) => {
			...
			<div { ...blockProps }>
				<div className="blog-listing-header">
					...
					<div className="blog-filters">
						<div className="blog-filters__dropdown">
							<label
								htmlFor="latest-blogs-tag-filter"
								...
								{ categories &&
									categories.map( ( cat ) => (
										<button
											key={ cat.id }
											type="button"
											className={ `tabs__tab text-md-Semibold tab-button ${
												! selectedTagId &&
				...
				<div className="ambrygen-ajax-pagination">
					<div className="ambrygen-ajax-pagination__content">
							...
							postsWithFullData.length > 0 && (
								<div className="blog-listing">
									{ postsWithFullData.map( ( post ) => {
										...
										const authorsData =
											getPostAuthorsData( post );
										const hasAuthors =
											authorsData.length > 0;
										const authorNames = hasAuthors
											? formatAuthorNames( authorsData )
											: '';
						...
						{ ! isFetching &&
							postsWithFullData &&
							postsWithFullData.length >= postsPerPage && (
								<>
									<div
