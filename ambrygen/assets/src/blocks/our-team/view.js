...
function initSliders( sliders ) {
	...
	sliders.forEach( ( sliderElement ) => {
		...
		const nextEl = showNavigation
			? sliderElement.querySelector( '.custom-next' )
			: null;
		const prevEl = showNavigation
			? sliderElement.querySelector( '.custom-prev' )
			: null;
		const paginationEl =
		...
		const autoplayConfig =
			config.autoplay && hasMultipleSlides && ! prefersReducedMotion
				? {
		...
		new Swiper( sliderElement, {
			...
			navigation:
				hasMultipleSlides && showNavigation && nextEl && prevEl
					? { nextEl, prevEl }
					: false,
			pagination:
				hasMultipleSlides && paginationEl
					? {
							el: paginationEl,
							clickable: true,
					  }
...
function initOurTeamBlocks() {
	...
	const teamBlocks = document.querySelectorAll(
