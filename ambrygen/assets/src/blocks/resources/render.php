...
if ( $ambrygen_enable_custom ) {
	foreach ( $ambrygen_custom_collaborators as $ambrygen_custom ) {
		...
		$ambrygen_custom_name      = isset( $ambrygen_custom['name'] ) ? sanitize_text_field( $ambrygen_custom['name'] ) : '';
		$ambrygen_custom_url       = isset( $ambrygen_custom['url'] ) ? (string) $ambrygen_custom['url'] : '';
		...
		if (
			'' === $ambrygen_custom_name &&
			'' === trim( $ambrygen_custom_url ) &&
			! $ambrygen_custom_image_id &&
			'' === trim( $ambrygen_custom_image_url )
		) {
			continue;
		}

		$ambrygen_valid_custom_collaborators[] = array(
			'name'      => $ambrygen_custom_name,
			'url'       => $ambrygen_custom_url,
			'target'    => isset( $ambrygen_custom['target'] ) ? sanitize_text_field( $ambrygen_custom['target'] ) : '',
			'rel'       => isset( $ambrygen_custom['rel'] ) ? sanitize_text_field( $ambrygen_custom['rel'] ) : '',
						...
						<div class="resources__card-logo-grid resources__card-logo-grid--3-col js-gsap-fade">
							...
							<?php if ( ! empty( $ambrygen_valid_custom_collaborators ) ) : ?>
								<?php foreach ( $ambrygen_valid_custom_collaborators as $ambrygen_custom ) : ?>
									...
									$ambrygen_custom_target     = '_blank' === ( $ambrygen_custom['target'] ?? '' ) ? '_blank' : '';
									$ambrygen_custom_rel        = '_blank' === $ambrygen_custom_target ? 'noopener noreferrer' : '';
								...
								<?php else : ?>
									<a href="<?php echo esc_url( $ambrygen_custom_url ); ?>" class="resources__card-logo-link" <?php echo $ambrygen_custom_target ? 'target="_blank" rel="' . esc_attr( $ambrygen_custom_rel ) . '"' : ''; ?> aria-label="<?php echo esc_attr( $ambrygen_custom_label ); ?>">
										<?php
										// phpcs:disable WordPress.Security.EscapeOutput.OutputNotEscaped -- Helper::image_from_source() escapes attributes and returns wp_kses_post()-sanitized image markup.
										echo Helper::image_from_source(
											$ambrygen_custom_image_id,
