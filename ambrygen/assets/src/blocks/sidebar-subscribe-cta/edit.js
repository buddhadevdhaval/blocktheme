		...
		return (
			<div { ...blockProps }>
				<Notice status="warning" isDismissible={ false }>
					{ __(
						'This block is only available for Blog Posts.',
						'ambrygen-web'
					) }
				</Notice>
			</div>
		...
		<div { ...blockProps }>
			...
			<div className="sidebar-widget subscribe-card-cta">
				<div className="subscribe-card-cta__content">
					<div className="subscribe-card-cta__title heading-5 mb-0">
						{ title || __( 'Love this article?', 'ambrygen-web' ) }
					</div>
					<div className="subscribe-card-cta__subtitle">
						{ subtitle ||
							__(
								'Get stories just like it, delivered right to you.',
								'ambrygen-web'
							) }
					</div>
				</div>
				...
				{ isTemplateEditor && (
					<Notice status="info" isDismissible={ false }>
						{ __(
							'Template preview: The subscribe form will be active on the frontend.',
							'ambrygen-web'
						) }
					</Notice>
				) }
