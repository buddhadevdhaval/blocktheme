...
<div <?php echo $ambrygen_wrapper_attributes; // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped -- Generated by get_block_wrapper_attributes(). ?>>
	<?php if ( ! empty( $ambrygen_tabs ) ) : ?>
		<div class="tabs__mobile-nav">
			<select class="tabs__select text-md-sbold" aria-label="<?php esc_attr_e( 'Select section', 'ambrygen-web' ); ?>">
				<?php foreach ( $ambrygen_tabs as $index => $tab_item ) : ?>
					...
					$is_selected = $is_active || 0 === $index;
					$option_text = $label ? $label : $target;

					if ( ! $target || ! $option_text ) {
						continue;
					}
					?>
					<option value="<?php echo esc_attr( $target ); ?>" <?php selected( $is_selected ); ?>>
