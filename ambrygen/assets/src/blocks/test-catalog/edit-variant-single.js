...
const getTermLabel = ( term, allTerms = [] ) => {
	...
	if ( term.parent && term.parent > 0 ) {
		const parent = allTerms.find(
			( t ) => Number( t.id ) === Number( term.parent )
		);
		if ( parent ) {
			const parentName = parent.name
				? decodeEntities( parent.name )
				: decodeEntities( parent.slug || '' );
			if ( parentName ) {
...
const getBaseVersionIds = ( selectedVersionIds, singleProductVersionId ) => {
	if ( selectedVersionIds.length > 0 ) {
		return [ ...selectedVersionIds ];
	}

	if ( singleProductVersionId ) {
		return [ Number( singleProductVersionId ) ];
	}

	return [];
};

const getVersionHelpText = ( versionsError, singleCategoryId ) => {
	if ( versionsError ) {
		return __(
	...
	if ( singleCategoryId ) {
		return __( 'Search product versions by title.', 'ambrygen-web' );
	}
...
export default function SingleVersionSettings( { attributes, setAttributes } ) {
	...
	const versionHelpText = getVersionHelpText(
		versionsError,
		singleCategoryId
	);

	const versionRows = useMemo( () => {
		const baseIds = getBaseVersionIds(
			selectedVersionIds,
		...
		const timeoutId = setTimeout( async () => {
			...
			try {
				...
				if ( isMounted ) {
					...
					const parentIdsToFetch = terms.reduce( ( ids, term ) => {
						if (
							term.parent > 0 &&
							! terms.some(
								( parentTerm ) => parentTerm.id === term.parent
							)
						) {
							ids.push( term.parent );
						}
							...
							.then( ( parentTerms ) => {
								if (
									...
									setCategoryOptions( ( current ) => {
										const existingIds = current.map(
											( c ) => c.id
										);
										const uniqueNew = parentTerms.filter(
											( p ) =>
	...
	useEffect( () => {
		...
		const selectedIds = selectedVersionIds.filter( ( id ) => id > 0 );
		const idsToEnsure =
			selectedIds.length > 0
				? selectedIds
				: getBaseVersionIds( [], singleProductVersionId );
		...
		const missingIds = idsToEnsure.filter(
			( id ) =>
				! versionOptions.some( ( post ) => Number( post.id ) === id )
		);
	...
	return (
		<PanelBody
			title={ __( 'Single Version Settings', 'ambrygen-web' ) }
			initialOpen
		>
			<ComboboxControl
				...
				help={
					categoriesError
						...
						: __(
								'Search categories to narrow the list (optional).',
								'ambrygen-web'
						  )
				}
			/>
			...
			<div style={ { marginTop: '12px' } }>
				...
				<ComboboxControl
					label={ __(
						'Legacy Single Product Version',
						'ambrygen-web'
					) }
					value={
						singleProductVersionId
							? String( singleProductVersionId )
							: ''
					}
					options={ versionComboOptions }
					onFilterValueChange={ setVersionSearch }
