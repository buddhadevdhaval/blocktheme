...
import { BlockExamplePreview, ItemHeader } from '../_shared/components';
...
function TabSettings( {
	...
	const { terms, parentTerms, selectedTerm, isLoadingTerms } = useSelect(
		( select ) => {
			const query = {
				per_page: 20,
				hide_empty: false,
			...
			const fetchedTerms = select( 'core' ).getEntityRecords(
				'taxonomy',
				'poster_category',
				query
			);
			...
			if ( Array.isArray( fetchedTerms ) && fetchedTerms.length > 0 ) {
				...
				if ( parentIds.length > 0 ) {
					fetchedParentTerms = select( 'core' ).getEntityRecords(
						'taxonomy',
						'poster_category',
						{ include: parentIds, _fields: 'id,name,slug,parent' }
					);
				}
			}
	...
	const termOptions = useMemo( () => {
		...
		if ( Array.isArray( parentTerms ) ) {
			combined.push( ...parentTerms );
		}
		if ( selectedTerm ) {
			combined.push( selectedTerm );
		}
	...
	const { availablePosts, isLoadingPosts } = useSelect(
		( select ) => {
			if ( ! tab?.termId ) {
			...
			const isResolving = select( 'core' ).isResolving(
				'getEntityRecords',
				[ 'postType', 'product_version', query ]
			);
		...
		>
			...
			{ selectedOption && ! searchInput && (
				<p style={ { marginTop: '8px', marginBottom: 0 } }>
					...
					<strong>
						{ getTermLabel( selectedOption, termOptions ) }
					</strong>
				</p>
			) }
