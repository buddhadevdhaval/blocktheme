document.addEventListener( 'DOMContentLoaded', () => {
	...
	const shouldReloadIframeForResize = ( iframe, viewportWidthChanged ) => {
		if ( ! iframe || ! viewportWidthChanged ) {
			return false;
		}

		const nextViewportMode = window.innerWidth < 768 ? 'mobile' : 'desktop';

		// Mobile browsers fire resize during scroll and keyboard changes.
		// Re-loading the iframe there causes the visible blinking issue.
		if ( nextViewportMode === 'mobile' ) {
			return false;
		}

		return iframe.dataset.viewportMode !== nextViewportMode;
	};
	...
	const getIframeHeightFromMessage = ( messageData ) => {
		...
		if ( typeof messageData === 'string' ) {
			const directHeightMatch = messageData.match(
				/^setHeight:(\d+)(?::.*)?$/i
			);
	...
	window.addEventListener( 'resize', () => {
		...
		viewportResizeTimer = window.setTimeout( () => {
			...
			document
				.querySelectorAll( '.theme-form-block__iframe' )
				.forEach( ( iframe ) => {
					if ( shouldReloadIframeForResize( iframe, viewportWidthChanged ) ) {
						reloadIframeForResize( iframe );
						return;
					}
