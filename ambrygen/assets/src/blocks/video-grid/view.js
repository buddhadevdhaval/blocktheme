( function () {
	const initVideoGridModal = () => {
		...
		document
			.querySelectorAll(
				'.wp-block-ambrygen-gallery.image-grid-block.video-grid'
			)
			.forEach( ( blockEl ) => {
				...
				const videoModal =
					blockEl.querySelector( '[data-video-modal]' );
				...
				const overlay = videoModal.querySelector(
				...
				const openVideoModal = ( element ) => {
					...
					const videoSrc =
						mediaContainer?.dataset?.videoSrc ||
						item
							.querySelector( '.features-media__iframe' )
							?.getAttribute( 'src' );
					const videoType =
						mediaContainer?.dataset?.videoType || 'embed';
				...
				const triggers =
					blockEl.querySelectorAll( '.open_video_popup' );
				triggers.forEach( ( trigger ) => {
					...
					const mediaEl = trigger.querySelectorAll(
