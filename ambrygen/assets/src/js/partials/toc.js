document.addEventListener('DOMContentLoaded', function () {
	...
	document.querySelectorAll('.combo-color-row h2, .combo-color-row h3, .combo-color-row h4, .combo-color-row h5').forEach(function (heading) {
	...
	if (blogHTML === '') {
		document.querySelectorAll('.sidebar-title, .sidebar-menu, .blog-detail-table-of-content').forEach(el => el.style.display = 'none');
	} else {
		sidebarMenus.forEach(menu => menu.innerHTML += blogHTML);
		if (mobileTOC) mobileTOC.innerHTML = blogHTML;
	}
	...
	window.addEventListener('scroll', function () {
		document.querySelectorAll('.jump-link-post').forEach(function (section) {
	...
	function updateTOC(id) {
		document.querySelectorAll('.sidebar-menu li, .dropdown-menu li').forEach(li => li.classList.remove('active'));
		document.querySelectorAll(`.sidebar-menu a[href="#${id}"], .dropdown-menu a[href="#${id}"]`).forEach(link => {
	...
	const toggleBtn = document.querySelector('.tab-dropdown .dropdown-toggle');
	const dropdownMenu = document.querySelector('.tab-dropdown .dropdown-menu');
