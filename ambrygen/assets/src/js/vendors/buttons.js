...
const { PanelBody, SelectControl, TextControl, Button, BaseControl, Popover } =
	wp.components;
		...
		return el(
			...
			el(
				...
				el(
					...
					el( SelectControl, {
						...
						options: [
							...
							{ label: 'Right Arrow', value: 'has-right-arrow' },
							{
								...
								value: 'has-download-arrow',
							},
							{
								label: 'Form Arrow',
								value: 'has-form-arrow has-right-arrow',
							},
							{
								label: 'Video Arrow',
								value: 'has-video-arrow has-right-arrow',
							},
						],
						onChange: ( val ) =>
							props.setAttributes( { buttonArrowIcon: val } ),
					} ),
					props.attributes.buttonArrowIcon.includes(
						'has-form-arrow'
					) &&
						el(
							Fragment,
					...
					props.attributes.buttonArrowIcon &&
						props.attributes.buttonArrowIcon.includes(
							'has-video-arrow'
						) &&
						el(
							...
							el( SelectControl, {
								...
								onChange: ( val ) =>
									props.setAttributes( { videoType: val } ),
							} ),
							props.attributes.videoType === 'embed' &&
								el( SelectControl, {
									label: 'Platform',
									value:
										props.attributes.videoPlatform ||
										'youtube',
									options: [
							...
							props.attributes.videoType === 'mp4'
								? el(
										BaseControl,
										{ label: 'Video File' },
										el(
							...
							el( TextControl, {
								...
								onChange: ( val ) =>
									props.setAttributes( { videoTitle: val } ),
							} ),
							el(
								Button,
								{
								...
								el(
									Popover,
									{
										anchor: document.getElementById(
											'ambrygen-video-content-btn'
										),
										onClose: () => setIsModalOpen( false ),
										position: 'bottom center',
										noArrow: false,
										className:
											'ambrygen-video-content-popover',
									},
									el(
										'div',
										{
										...
										el( RichText, {
											...
											placeholder:
												'Enter Video content (links supported)...',
											allowedFormats: [
												'core/bold',
												'core/italic',
												'core/link',
...
addFilter(
	'editor.BlockEdit',
	'ambrygen/with-arrow-icon-control',
	withArrowIconControl
);
...
addFilter(
	...
	( extraProps, blockType, attributes ) => {
		if ( blockType.name === 'core/button' && attributes.buttonArrowIcon ) {
			...
			if (
				attributes.buttonArrowIcon &&
				attributes.buttonArrowIcon.includes( 'has-video-arrow' )
			) {
				let videoSrc =
				...
				if (
					attributes.videoType === 'embed' &&
					attributes.iframeUrl
				) {
					const prefix =
						attributes.videoPlatform === 'vimeo'
							? 'https://player.vimeo.com/video/'
							: 'https://www.youtube.com/embed/';
			...
			if (
				attributes.buttonArrowIcon &&
				attributes.buttonArrowIcon.includes( 'has-form-arrow' )
			) {
				extraProps[ 'data-form-title' ] =
					attributes.formPopupTitle || 'Coming soon';
			}
		}
		return extraProps;
	}
...
const withArrowIconClassEditor = createHigherOrderComponent(
	( BlockListBlock ) => {
		return ( props ) => {
			if (
				...
				const customClassName = props.attributes.buttonArrowIcon;
...
addFilter(
	...
	'ambrygen/editor-button-arrow-class',
	withArrowIconClassEditor
);
