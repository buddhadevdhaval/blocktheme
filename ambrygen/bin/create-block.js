 ...
 * @param {string} name
 * @return {string} The validated block name
 */
 ...
 * @param {string[]} args
 * @return {{parsed: Record<string, string>, remaining: string[]}} Parsed arguments and remaining args
 */
...
async function main() {
	try {
		...
		if ( await directoryExists( blockDir ) ) {
			throw new Error(
				`Block "${ blockName }" already exists at ${ blockDir }`
			);
		}
