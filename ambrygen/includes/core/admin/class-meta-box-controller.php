...
final class MetaBoxController
{
    ...
    public function render_post_author_meta_box(WP_Post $post): void
    {
        ...
        $key = 'linked_author';
        $field = [
            'label'       => __('Author(s)', 'ambrygen-web'),
            'type'        => 'post_relationship',
            'description' => __('Select one or more authors to link to this post.', 'ambrygen-web'),
            'multiple'    => true,
            'post_types'  => ['author'],
        ];
    ...
    public function render_meta_box(WP_Post $post, array $box): void
    {
        ...
        foreach ($fields as $key => $field) {
            ...
            } elseif ('poster_pdf_repeater' === $type) {
    ...
    private function render_media_file_field(int $post_id, string $key, array $field, string $value): void
    {
        ...
        echo '</div>';
        echo '<div class="form-field form-field-button">';
        echo '<button type="button" class="button button-secondary ambrygen-media-file-upload">';
        esc_html_e('Select File', 'ambrygen-web');
        echo '</button> ';
        echo '<button type="button" class="button button-secondary ambrygen-media-file-remove">';
        esc_html_e('Clear', 'ambrygen-web');
        echo '</button>';
        echo '</div>';
        echo '</div>';
    ...
    public function save_meta_boxes(int $post_id, WP_Post $post): void
    {
        ...
        if ('post' === $post->post_type || 'webinar' === $post->post_type) {
    ...
    private function save_marketing_material_repeater(int $post_id, string $key, array $field): void
    {
        ...
        $allowed_statuses = ['in_production', 'on_staging', 'disabled_urgent'];
    ...
    public function enqueue_post_meta_media(): void
    {
        ...
        if (! $screen || ! in_array($screen->post_type, array_keys($this->definitions), true)) {
            return;
        }
