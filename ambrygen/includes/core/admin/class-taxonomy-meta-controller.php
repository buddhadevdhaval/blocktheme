...
/**
 * Taxonomy meta registration and admin UI controller.
 */
final class TaxonomyMetaController {
	 ...
	 * Constructor.
	 *
	 * @param array $taxonomies Taxonomy definitions keyed by slug.
	 */
	...
	public function set_taxonomies( array $taxonomies ): void {
		...
		foreach ( $taxonomies as $key => $value ) {
			if ( is_array( $value ) ) {
				...
				$this->taxonomies[ $value ] = array( 'slug' => $value );
			}
		}
		$this->register_hooks();
	}
	...
	private function register_hooks(): void {
		foreach ( $this->taxonomies as $slug => $tax ) {
				...
				foreach ( $tax['meta_fields'] as $meta_key => $field ) {
					...
					register_term_meta(
						...
						array(
							'type'              => $meta_type,
							'single'            => $is_single,
							'show_in_rest'      => $this->get_rest_schema( $field, $is_single, $meta_type ),
			...
			add_action( $slug . '_add_form_fields', array( $this, 'render_add_form_fields' ) );
			add_action( $slug . '_edit_form_fields', array( $this, 'render_edit_form_fields' ), 10, 2 );
			add_action( 'created_' . $slug, array( $this, 'save_term_meta' ), 10, 2 );
			add_action( 'edited_' . $slug, array( $this, 'save_term_meta' ), 10, 2 );
		}

		add_action( 'admin_enqueue_scripts', array( $this, 'enqueue_scripts' ) );
		add_action( 'admin_notices', array( $this, 'render_collaborator_export_notice' ) );
		add_action( 'admin_init', array( $this, 'handle_collaborator_export' ) );
	}

	/**
	 * Get the current taxonomy edit screen when available.
	 *
	...
	/**
	 * Render the collaborator export admin notice.
	 *
	 * @return void
	 */
	...
	/**
	 * Handle collaborator CSV export requests.
	 *
	 * @return void
	 */
	public function handle_collaborator_export(): void {
		...
		fputcsv( $output, array( '_old_id', 'name' ) );
	...
	/**
	 * Get the sanitize callback for a taxonomy meta field.
	 *
	...
	private function get_sanitize_callback( array $field, bool $is_single ): callable {
		...
		if ( ! $is_single ) {
			return array( $this, 'sanitize_item_array' );
		}
	...
	/**
	 * Build the REST schema definition for a taxonomy meta field.
	 *
	 * @param array  $field     Meta field definition.
	 * @param bool   $is_single Whether the field stores a single value.
	 * @param string $meta_type Registered meta type.
	 * @return array|bool
	 */
	private function get_rest_schema( array $field, bool $is_single, string $meta_type ) {
		if ( $is_single || 'array' !== $meta_type ) {
			return true;
		}
		...
		return array(
			'schema' => array(
				'type'  => 'array',
				'items' => array(
					'type' => $this->get_rest_item_type( $field ),
				),
			),
		);
	}
	...
	/**
	 * Determine the REST item type for an array taxonomy meta field.
	 *
	 * @param array $field Meta field definition.
	 * @return string
	 */
	private function get_rest_item_type( array $field ): string {
		$sanitize = $field['sanitize'] ?? null;

		if ( in_array( $sanitize, array( 'absint', 'intval' ), true ) ) {
			return 'integer';
		}
	...
	/**
	 * Sanitize an array of taxonomy meta values.
	 *
	...
	/**
	 * Render add-term form fields for supported taxonomies.
	 *
	 * @return void
	 */
	...
	/**
	 * Render edit-term form fields for supported taxonomies.
	 *
	 * @param mixed $term Term object being edited.
	 * @return void
	 */
	...
	/**
	 * Save taxonomy term meta fields.
	 *
	...
	/**
	 * Enqueue taxonomy term media scripts.
	 *
	 * @return void
	 */
