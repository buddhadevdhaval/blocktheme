...
/**
 * Relationship field renderer for linking posts in admin.
 */
final class PostRelationshipField {
	...
	/**
	 * Render the post relationship field.
	 *
	...
	public function render( int $post_id, string $key, array $field ): void {
		...
		$allowed_post_types = isset( $field['post_types'] ) ? $field['post_types'] : array();
		...
		if ( ! empty( $linked_posts ) ) {
			...
			foreach ( $linked_posts as $linked_post_id ) {
				...
				if ( $linked_post ) {
					...
					$post_status_label = $this->get_post_status_label( $linked_post->post_status );
					...
					printf(
						'<li>%1$s <em>(%2$s | %3$s)</em>%4$s <a href="#" class="ambrygen-remove-link" data-post-id="%5$s" style="color:#dc3232;text-decoration:none;">[%6$s]</a></li>',
		...
		echo '<div class="ambrygen-add-post-relationship" style="margin-top: 15px;">';
		...
		echo '<div class="ambrygen-search-results search-filter-lists" style="max-height:250px;overflow-y:auto;display:none;border:1px solid #ddd;padding:20px;background:#f9f9f9;"></div>';
