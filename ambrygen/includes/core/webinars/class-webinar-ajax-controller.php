...
/**
 * AJAX controller for webinar archive pagination updates.
 */
final class WebinarAjaxController {

	use Singleton;

	/**
	 * Register webinar AJAX hooks.
	 */
	...
	/**
	 * Register AJAX actions for webinar archive pagination.
	 */
	private function register_hooks(): void {
		add_action( 'wp_ajax_ambrygen_webinar_pagination', array( $this, 'handle_pagination' ) );
		add_action( 'wp_ajax_nopriv_ambrygen_webinar_pagination', array( $this, 'handle_pagination' ) );
	}

	/**
	 * Handle webinar archive pagination requests.
	 */
	public function handle_pagination(): void {
		...
		$per_page = $per_page > 1 ? $per_page : 8;
	...
	/**
	 * Render webinar archive markup from the HTML template parts.
	 *
	...
	private function render_content_from_template( string $scope, int $paged, int $per_page = 8, string $s = '', int $total_pages = 1, int $total_posts = 0, int $year = 0, int $tag = 0 ): string {
		$part = 'past' === $scope ? 'parts/past-webinars.html' : 'parts/upcoming-webinars.html';
		...
		$pagination_block = $this->find_first_block_by_name( $blocks, 'ambrygen/webinar-archive-pagination' );
		if ( ! $query_block ) {
			return '';
		}
		...
		if ( $total_posts === 0 ) {
			return '<div class="no-results-message text-center no-result-alert">No Webinars found</div>';
		}
	...
	/**
	 * Find the first query block in parsed block markup.
	 *
	...
	/**
	 * Find the first block matching a given block name.
	 *
	 * @param array  $blocks     Parsed block list.
	...
	/**
	 * Replace static pagination markup with dynamic archive state.
	 *
	...
	/**
	 * Build numbered pagination button markup.
	 *
	 * @param int $paged       Current page.
	 * @param int $total_pages Total pages.
	 * @return string
	...
	/**
	 * Build popup pagination grid markup.
	 *
	 * @param int $paged       Current page.
	 * @param int $total_pages Total pages.
	 * @return string
	...
	/**
	 * Update the per-page dropdown state in rendered archive HTML.
	 *
	 * @param string $html     Rendered HTML.
	 * @param int    $per_page Selected per-page value.
	 * @param string $suffix   Scope-specific element suffix.
	 * @return string
	 */
	...
	/**
	 * Update the year dropdown state in rendered archive HTML.
	 *
	 * @param string $html Rendered HTML.
